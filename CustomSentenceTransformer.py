...
class SentenceTransformer(nn.Sequential):
    ...
    def fit(self,
        ...
        if freeze:
            ...
            self._first_module().auto_model.requires_grad_(False)
            last_layer_num = len(self._first_module().auto_model.encoder.layer) - 1
            # last_layer_num = len(self._first_module().auto_model.transformer.layer) - 1 # for distil models

            for param in (
                self._first_module()
                .auto_model.encoder.layer[last_layer_num]
                # .auto_model.transformer.layer[last_layer_num]
                .parameters()
            ):
