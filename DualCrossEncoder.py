...
class CrossEncoder:
    def __init__(
        ...
        qnli_model_name: str,
        sts_model_name: str,
        qnli_pretrained_path: str,
        sts_pretrained_path: str,
        num_labels: int = None,
        max_length: int = None,
        device: str = None,
        tokenizer_args: Dict = {},
        ...
        self.config = AutoConfig.from_pretrained(qnli_model_name)
        ...
        self.model = DualBERT(qnli_model_name, sts_model_name)
        load_model(self.model.qnli_bert, qnli_pretrained_path)
        load_model(self.model.sts_bert, sts_pretrained_path)

        self.qnli_tokenizer = AutoTokenizer.from_pretrained(
            qnli_model_name, **tokenizer_args
        )
        self.sts_tokenizer = AutoTokenizer.from_pretrained(
            sts_model_name, **tokenizer_args
        )
    ...
    def smart_batching_collate(self, batch):
        qa_texts = [[] for _ in range(len(batch[0].qa_sentence_pair))]
        raa_texts = [[] for _ in range(len(batch[0].raa_sentence_pair))]
        scores = []
        sources = []

        for example in batch:
            for idx, (qa_text, raa_text) in enumerate(
                zip(example.qa_sentence_pair, example.raa_sentence_pair)
            ):
                qa_texts[idx].append(qa_text.strip())
                raa_texts[idx].append(raa_text.strip())

            scores.append(example.score)
            sources.append(example.dataset)

        qa_tokenized = self.qnli_tokenizer(
            *qa_texts,
            padding=True,
        ...
        raa_tokenized = self.sts_tokenizer(
            *raa_texts,
            padding=True,
            truncation="longest_first",
            return_tensors="pt",
        ...
        for name in qa_tokenized:
            qa_tokenized[name] = qa_tokenized[name].to(self._target_device)
        for name in raa_tokenized:
            raa_tokenized[name] = raa_tokenized[name].to(self._target_device)

        return qa_tokenized, raa_tokenized, scores, sources
    ...
    def smart_batching_collate_text_only(self, batch):
        ...
        for example in batch:
            qa_texts[0].append(example[0].strip())  # question
            raa_texts[0].append(example[1].strip())  # reference answer
            qa_texts[1].append(example[2].strip())  # answer
            raa_texts[1].append(example[2].strip())  # answer
        ...
        return qa_tokenized, raa_tokenized
    ...
    def fit(
        ...
        if freeze:
            ...
            self.model.qnli_bert.requires_grad_(False)
            self.model.sts_bert.requires_grad_(False)
        ...
        for epoch in trange(epochs, desc="Epoch", disable=not show_progress_bar):
            ...
            for qa_features, raa_features, scores, sources in tqdm(
                train_dataloader,
                desc="Iteration",
                smoothing=0.05,
                ...
                if use_amp:
                    with autocast():
                        model_predictions = self.model(qa_features, raa_features)
                        logits = activation_fct(model_predictions)
                        if self.config.num_labels == 1:
                            logits = logits.view(-1)
                        loss_value = criterion(logits, scores, sources)
    ...
    def predict(
        ...
        self.model.to(self._target_device)
        with torch.no_grad():
            for qa_features, raa_features in iterator:
                model_predictions = self.model(qa_features, raa_features)
                logits = activation_fct(model_predictions)
