...
class EnsembleEncoder:
    def __init__(
        ...
        ce_model_name = "cross-encoder/stsb-roberta-base"
        st_model_name = "sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2"
        foss_model_name = "avsolatorio/GIST-Embedding-v0"

        self.config = AutoConfig.from_pretrained(ce_model_name)
        ...
        self.model = HybridEnsemble(ce_model_name, st_model_name, foss_model_name)
        load_model(self.model.CE_roberta, cross_encoder_path)
        load_model(self.model.multilingual_st, st_encoder_path)
        load_model(self.model.gist_bert, foss_encoder_path)

        self.cross_encoder_tokenizer = AutoTokenizer.from_pretrained(
            ce_model_name, **tokenizer_args
        )
        self.st_tokenizer = AutoTokenizer.from_pretrained(
            st_model_name, **tokenizer_args
        )
        self.gist_tokenizer = AutoTokenizer.from_pretrained(
            foss_model_name, **tokenizer_args
        )
    ...
    def smart_batching_collate(self, batch):
        texts = [[] for _ in range(len(batch[0].sentence_pair))]
        reference_answers = []
        student_answers = []
        scores = []
        sources = []
        ...
        for example in batch:
            for idx, text in enumerate(example.sentence_pair):
                texts[idx].append(text.strip())

            reference_answers.append(example.sentence_pair[0].strip())
            student_answers.append(example.sentence_pair[1].strip())
            scores.append(example.score)
            sources.append(example.dataset)

        ce_tokenized = self.cross_encoder_tokenizer(
            *texts,
            padding=True,
        ...
        st_tokenized_0 = self.st_tokenizer(
            reference_answers,
            padding=True,
            truncation=True,
            return_tensors="pt",
        ...
        st_tokenized_1 = self.st_tokenizer(
            student_answers,
            padding=True,
            truncation=True,
            return_tensors="pt",
        ...
        gist_tokenized_0 = self.gist_tokenizer(
            reference_answers,
            padding=True,
            truncation=True,
            return_tensors="pt",
        ...
        gist_tokenized_1 = self.gist_tokenizer(
            student_answers,
            padding=True,
            truncation=True,
            return_tensors="pt",
        ...
        scores = torch.tensor(
            scores, dtype=torch.float if self.config.num_labels == 1 else torch.long
        ).to(self._target_device)

        for name in ce_tokenized:
            ce_tokenized[name] = ce_tokenized[name].to(self._target_device)
        for name in st_tokenized_0:
            st_tokenized_0[name] = st_tokenized_0[name].to(self._target_device)
        for name in st_tokenized_1:
            st_tokenized_1[name] = st_tokenized_1[name].to(self._target_device)
        for name in gist_tokenized_0:
            gist_tokenized_0[name] = gist_tokenized_0[name].to(self._target_device)
        for name in gist_tokenized_1:
            gist_tokenized_1[name] = gist_tokenized_1[name].to(self._target_device)
    ...
    def smart_batching_collate_text_only(self, batch):
        ...
        for example in batch:
            ...
            reference_answers.append(example[0].strip())
            student_answers.append(example[1].strip())
    ...
    def fit(
        ...
        if freeze:
            ...
            self.model.multilingual_st.requires_grad_(False)
            self.model.gist_bert.requires_grad_(False)
            self.model.CE_roberta.requires_grad_(False)
    ...
    def save(self, path):
        ...
        # self.qnli_tokenizer.save_pretrained(path)
        # self.sts_tokenizer.save_pretrained(path)
